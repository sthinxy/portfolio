...
(function () {
  ...
  const editableTextKeys = [
    "gate.top","gate.frame","gate.label","gate.intro","gate.desc","gate.placeholder","gate.enter","gate.hint",
    ...
    "dialog.welcome","dialog.toBlack","dialog.toWhite","dialog.preyCaught","foot.by","foot.hint","foot.hint2"
  ];
  ...
  function fillForm(){
    ...
    $("#skillsJson").value = stableJson(content.skills || []);
    $("#projectFiltersJson").value = stableJson(content.projectFilters || []);
    $("#projectsJson").value = stableJson(content.projects || []);
    $("#mobileDockJson").value = stableJson(content.settings?.mobileDock || []);
  ...
  function collectForm(fromFullJson=false){
    ...
    content.settings.mobileDock = parseJson("#mobileDockJson");
    content.services = parseJson("#servicesJson");
    content.skills = parseJson("#skillsJson");
    content.projectFilters = parseJson("#projectFiltersJson");
  ...
  function initAssetUploader(){
    ...
    input?.addEventListener("change", async e => {
      ...
      if(!file.type.startsWith("image/")){
        status("Selecione um arquivo de imagem.", true);
        return;
      }
      try{
        ...
        status(`imagem carregada: ${file.name}. Escolha onde aplicar e clique em ANEXAR.`);
      }catch(err){
        status(err.message || "não consegui ler a imagem.", true);
      }
    });

    attach?.addEventListener("click", () => {
      if(!pendingDataUrl){
        status("Escolha uma imagem antes de anexar.", true);
        return;
      }
      try{
        collectForm($(".tab.active")?.dataset.tab === "json");
        setPath(content, target?.value || "design.portraitWhite", pendingDataUrl);
        fillForm();
        const field = $(`[data-path="${target?.value || "design.portraitWhite"}"]`);
        if(field) field.value = pendingDataUrl;
        $("#fullJson").value = stableJson(content);
        status("imagem anexada. Clique em SALVAR ALTERAÇÕES para aplicar.");
      }catch(err){
        status(err.message || "erro ao anexar imagem.", true);
      }
    });
  }
